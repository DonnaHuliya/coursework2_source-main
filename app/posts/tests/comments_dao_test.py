...
class TestCommentsDAO:
    ...
    def test_get_by_post_pk_check_type(self, comments_dao):
        comments_dao = comments_dao.get_by_post_pk(1)
        assert type(comments_dao) == list, "Результат должен быть списком"
        assert type(comments_dao[0]) == dict, "Результат должен быть словарем"
    ...
    def test_get_by_post_pk_check_keys(self, comments_dao, keys_expected):
        ...
        assert comment_keys == keys_expected, "Список ключей не соответсвует"
